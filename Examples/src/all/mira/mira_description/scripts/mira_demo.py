...
class MiraJointMover(object):
    ...
    def __init__(self):
        ...
        self.pub_mira_yaw_joint_position = rospy.Publisher('/mira/yaw_joint_position_controller/command',
                                                           Float64,
                                                           queue_size=1)
        self.joint_states_topic_name = "/mira/joint_states"
        mira_joints_data = self._check_joint_states_ready()
        if mira_joints_data is not None:
            self.mira_joint_dictionary = dict(zip(mira_joints_data.name, mira_joints_data.position))
            rospy.Subscriber(self.joint_states_topic_name, JointState, self.mira_joints_callback)
    ...
    def mira_movement_sayno(self):
        ...
        for repetition in range(2):
            ...
            while not similar:
                ...
                # This is because when publishing a topic, the first ties it doesnt work.
